...
const BUTTON_SEND_LIVE_OPTOUT_TEST = 'Send live opt-out test';
const BUTTON_CHECK_CUSTOMER_STATUS = 'Check customer status';
const BUTTON_RESTORE_CUSTOMER_OPTIN = 'Restore customer opt-in';
const LIVE_OPTOUT_TEST_WARNING = 'The live test email goes only to the store owner, but its opt-out link represents the customer attached to this order. Clicking that link will opt out that customer.';
const CUSTOMER_STATUS_OPTED_OUT = '%s (customer ID %d) is opted out.';
const CUSTOMER_STATUS_OPTED_IN = '%s (customer ID %d) has not opted out.';
...
const SUCCESS_LIVE_OPTOUT_TEST_SENT = 'Live opt-out test sent to %s using order #%d. The email represents %s (customer ID %d). Clicking its opt-out link will change that customer preference.';
const SUCCESS_CUSTOMER_RESTORED = '%s (customer ID %d) was restored and is eligible for review reminders.';
