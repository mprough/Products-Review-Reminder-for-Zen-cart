...
?>
	...
	function change_rating( pid ){
		...
		for( var i=1; i <= rating; i++ ){
			stars += '<img src="<?php echo DIR_WS_IMAGES; ?>review_reminder/rating_star_on.png" id="' + pid + '-' + i + '" onmouseover="show_rating(' + pid + ', ' + i + ');" onmouseout="hide_rating(\'' + pid + '\');" onclick="update_rating(\'' + pid + '\', ' + i + ');" /> ';
		}
		for( i; i <= 5; i++ ){
			stars += '<img src="<?php echo DIR_WS_IMAGES; ?>review_reminder/rating_star.png" id="' + pid + '-' + i + '" onmouseover="show_rating(' + pid + ', ' + i + ');" onmouseout="hide_rating(\'' + pid + '\');" onclick="update_rating(\'' + pid + '\', ' + i + ');" /> ';
		}
	...
	function update_stars(pid, rating){
		for(var i=1; i <= rating; i++){
			$('#' + pid + '-' + i).attr("src","<?php echo DIR_WS_IMAGES; ?>review_reminder/rating_star_on.png");
		}
		for(i; i <= 5; i++){
			$('#' + pid + '-' + i).attr("src","<?php echo DIR_WS_IMAGES; ?>review_reminder/rating_star.png");
		}
	}
	...
	function get_stars(rating){
		var stars = '';
		for(var i=1; i <= rating; i++){
			stars += '<img src="<?php echo DIR_WS_IMAGES; ?>review_reminder/rating_star_on.png" /> ';
		}
		for(i=rating; i < 5; i++){
			stars += '<img src="<?php echo DIR_WS_IMAGES; ?>review_reminder/rating_star.png" /> ';
		}
		return stars;
	}
