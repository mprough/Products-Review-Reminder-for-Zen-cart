...
<?php
	...
	function get_request_for_review_content($oid, $limit_products, $mailto, $customer_name, $customer_id, $is_test = false){
		...
		$html = '<div style="max-width:600px;margin:0 auto;font-family:Arial,sans-serif;color:#333">' . $test_html . '<p>' . htmlspecialchars($greeting, ENT_QUOTES, CHARSET) . '</p><p>' . nl2br(htmlspecialchars($intro, ENT_QUOTES, CHARSET)) . '</p><table style="width:100%;border-collapse:collapse"><tbody>' . "\r\n";
	...
	<tr>
		...
		$optout_html = '<p style="font-size:11px;color:#777">Opt-out link omitted from this test.</p>';
		$optout_text = 'Opt-out link omitted from this test.';
		if (!$is_test) {
