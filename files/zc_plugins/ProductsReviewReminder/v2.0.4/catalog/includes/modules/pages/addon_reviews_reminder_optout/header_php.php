...
if ($customerId > 0 && preg_match('/^[a-f0-9]{64}$/', $token)) {
    $customer = $db->Execute(
    ...
    if (!$customer->EOF) {
        $expected = hash_hmac(
            'sha256',
            $customerId . '|' . strtolower((string)$customer->fields['customers_email_address']),
            (string)STORE_OWNER_EMAIL_ADDRESS
        );
...
if (empty($reviewReminderOptedOut)) {
