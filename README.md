...
Products Review Reminder helps a shop owner find eligible completed orders and manually send customers a request for honest product feedback. Version 2.0.7 is maintained by Melanie Prough of [PRO-Webs, Inc.](https://pro-webs.net/).
...
6. Install version 2.0.7 through **Modules > Plugin Manager**.
...
The test panel includes an HTML or Plain text format switch. HTML uses Zen Cart's admin-copy HTML path, while Plain text deliberately suppresses the HTML block. The on-page preview remains HTML. The selected order customer's email preference applies only when a real reminder is sent to that customer.
